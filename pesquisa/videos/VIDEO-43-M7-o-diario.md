# Vídeo 43 de 46 — O diário de 9 campos: ele ajuda a seguir a regra, não a ganhar dinheiro

**Módulo 7 — A rotina · Aba "O diário" · Duração-alvo: 4 a 5 minutos (máximo 6)**
...
As duas frases lado a lado são o centro: "o registro ajuda você a seguir a sua regra" (tem lastro) × "o registro faz ganhar dinheiro" (não foi medido). Traduzir o efeito de 0,40 em chances (61%). Os 9 campos viram uma tela cada, com o campo 7 (saída executada) destacado como a peça que torna o campo 8 verificável.

**Conexões:** Assume o vídeo da regra. Prepara o vídeo da revisão.
...
- **d = 0,40** — Efeito de monitorar o progresso sobre cumprir metas. Meta-análise de 138 experimentos com 19.951 pessoas (Harkin et al., 2016). Pequeno a médio.
- **61%** — Chance de quem monitorou ter ido melhor que quem não monitorou. Contra 50% se não houvesse efeito.
- **Nenhum** — Estudos revisados por pares mostrando que diário dá lucro. O diário melhora a aderência à regra, não a qualidade da regra.
...
|  | Tipo | Por que existe |
|---|---|---|
| **1. A regra, escrita antes da entrada** | Comportamento | Sem ela, não dá para separar decisão de resultado. |
| **2. Data e hora, token e tamanho em % do capital** | Comportamento | O tamanho é parte da regra, não do resultado. |
| **3. Preço de entrada, saída por perda programada e alvo** | Comportamento | O plano de saída, registrado antes de saber como termina. |
| **4. A tese, em uma frase** | Comportamento | Congela a narrativa antes do resultado (a ficha do Módulo 4). |
| **5. Convicção, numa escala fixa de 1 a 5** | Comportamento | Deixa ver depois se convicção alta acerta mais — ou só aposta mais. |
| **6. Estado emocional na entrada** | Comportamento | Pressa, medo de ficar de fora, vontade de recuperar uma perda. |
| **7. Saída executada: hora, preço e o que disparou** | Comportamento | Stop, alvo, tempo ou decisão na hora. É o que torna o campo 8 verificável. |
| **8. Segui a regra?** | Comportamento | Sai da comparação entre os campos 1 e 3 e o campo 7 — não da memória. |
| **9. Resultado em dinheiro** | Resultado | O único campo de resultado. Olhe pouco (aba A revisão). |
