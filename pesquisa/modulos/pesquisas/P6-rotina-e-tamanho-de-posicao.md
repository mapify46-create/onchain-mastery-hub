...
Fundamentado em: (a) pré-registro/hipótese antes (Gollwitzer & Sheeran 2006, implementation intentions; prática de pré-registro AsPredicted/OSF/Registered Reports já estabelecida na sua pesquisa anterior); (b) distinção de Harkin entre monitorar **comportamento** vs. **resultado**; (c) distinção decisão boa vs. resultado bom (análise de decisão / "resulting" — Annie Duke é **[livro de mercado]**, marcar como tal).

Campos mínimos para medir as próprias decisões depois:
...
6. **Estado emocional** no momento.
7. **"Segui a regra?" (on-plan / off-plan)** — este é o campo de COMPORTAMENTO que Harkin sustenta monitorar.
8. **Resultado** (P&L) — campo de RESULTADO, a ser olhado com menos frequência (ver seção 4.2).

A chave metodológica: registre o **comportamento** (segui a regra?) separado do **resultado** (ganhei?), porque só o primeiro está sob seu controle e só o primeiro tem lastro em Harkin.
...
- run_blocking_subagent (Kelly-heavy-tail-math): f*=μ/σ², Bamberg & Neuhierl, gambler's ruin (Feller), Lo (2002) standard error
- enrich_draft (verificação de claims quantitativos)
...
- Adote os 9 campos da seção 1.4, com a **regra escrita ANTES da entrada** e o campo **"segui a regra? (on/off-plan)"** separado do P&L. Lastro: Harkin (monitorar comportamento) + implementation intentions (escrever antes).
